...
object Mst_animal {
 ...
 def time[A](f: => A) = {
  ...
  def main(args:Array[String]){
    val sc = new SparkContext(new SparkConf().setMaster("local").setAppName("Mst_animal"))
  val dist2 = sc.textFile("animal_distances.txt") 
val verts2 = dist2.map(_.split(",")(0)).distinct. 
    map(x => (x.hashCode.toLong,x)) 
val edges2 = dist2.map(x => x.split(",")). 
    map(x => Edge(x(0).hashCode.toLong,
                 x(1).hashCode.toLong,  
                 x(2).toDouble)) 
val distg2 = Graph(verts2, edges2) 
val mst2 = minSpanningTree(distg2)
println(time(minSpanningTree(distg2)))
val pw2 = new java.io.PrintWriter("animal.gexf") 
pw2.write(toGexf(mst2)) 
pw2.close
  }
}